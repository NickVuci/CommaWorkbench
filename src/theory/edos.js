...
import { centsFromMonzo } from '../core/monzo.js';
...
export function edoApprox(monzo, edo){
  const cents = centsFromMonzo(monzo);
  const edoCents = 1200 / edo;
  const steps = Math.round(cents / edoCents);
  const approxCents = steps * edoCents;
  const centsError = cents - approxCents;
  return { steps, approxCents, centsError };
}
